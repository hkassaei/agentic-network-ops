    ...
    def check_stack_rules(self, observations: dict) -> list[dict]:
        ...
        for rule in rules:
            ...
            if rule_id == "network_fault_is_root_cause":
                ...
                # Delegated to the pure-Python evaluator under
                # `network_ontology.rules.upf_directional`. The
                # evaluator handles BOTH cumulative-pair and rate-pair
                # fire conditions — when both pairs are in the
                # observations dict, BOTH verdicts fire (one each,
                # tagged with window_kind=cumulative|rate) so the
                # agent sees the analysis on both surfaces. Single
                # source of truth for the rule logic; consumed by
                # this method AND by `get_dp_quality_gauges` directly.
                # See ADR `upf_directional_rates_in_dp_quality_gauges.md`.
                from .rules import evaluate_upf_directional_rule
                triggered.extend(
                    evaluate_upf_directional_rule(
                        observations,
                        rule_prose=rule,
                    )
                )
