"""
Investigation Director v4 — topology-aware, context-isolated multi-agent pipeline.

Same pipeline as v3 but with network topology awareness: the triage agent
sees the full network graph (nodes, edges, link status) as its first tool
call, giving it immediate visibility into broken paths and degraded links.

Pipeline:
  Phase 0: Triage      → Session A → state["triage"]  (now topology-aware)
...
log = logging.getLogger("v4.orchestrator")
...
async def investigate(question: str, on_event=None) -> dict:
    ...
    log.info("Starting v4 investigation: %s", question[:100])
