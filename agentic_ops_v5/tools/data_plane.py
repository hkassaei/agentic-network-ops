...
async def _prom_query(client: httpx.AsyncClient, url: str, query: str) -> float:
...
async def get_dp_quality_gauges(window_seconds: int = _DEFAULT_WINDOW_SECONDS) -> str:
    """Get real-time data plane quality gauges over the last window_seconds.
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            values = await asyncio.gather(
                *(_prom_query(client, prom_url, q) for q in queries.values())
            )
    ...
    rtp_mos = _safe_div(results["rtp_mos_rate"], results["rtp_mos_samples_rate"])
    rtp_loss = _safe_div(results["rtp_loss_rate"], results["rtp_loss_samples_rate"])
    rtp_jitter = _safe_div(results["rtp_jitter_rate"], results["rtp_jitter_samples_rate"])
    ...
    lines = [
        f"Data Plane Quality Gauges (last {window_seconds}s):",
        "",
        "  RTPEngine:",
