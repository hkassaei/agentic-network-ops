...
if docker ps --format '{{.Names}}' | grep -q "^pcscf$"; then
    ...
    docker cp "$REPO_ROOT/network/pcscf/kamailio_pcscf.cfg" pcscf:/etc/kamailio_pcscf/kamailio_pcscf.cfg
    echo "  Restoring P-CSCF config (WITH_IPSEC enabled)..."
    docker cp "$REPO_ROOT/network/pcscf/pcscf.cfg" pcscf:/mnt/pcscf/pcscf.cfg
    docker restart pcscf
fi

if docker ps --format '{{.Names}}' | grep -q "^scscf$"; then
    echo "  Restoring S-CSCF config (HSS-Selected auth)..."
    docker cp "$REPO_ROOT/network/scscf/scscf.cfg" scscf:/mnt/scscf/scscf.cfg
