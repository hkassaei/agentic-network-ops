...
When forming a hypothesis that names an NF as the fault source, call `get_canonical_flows_through_component(nf)` to understand which canonical procedures touch it — this establishes what *else* should be broken if the hypothesis is true (useful for writing `supporting_events` and for ranking specificity). The output's `source` and `scope` fields say "NOT live deployment state"; that's the right mode for NA — you reason about which flows the hypothesis predicts will fail, not which are currently in flight (the Investigator handles live-activity questions via `get_active_flows_through_component`). Use `list_flows()` to discover available flow ids if you don't know them. Do not walk full flows yourself — that's the Investigator's job; stay at the overview level.
