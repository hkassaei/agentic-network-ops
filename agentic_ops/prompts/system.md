...
## Step 2: Check the IMS signaling path for the affected flow

**For call failures:** Trace the SIP INVITE through the IMS signaling chain.
- Check P-CSCF, I-CSCF, and S-CSCF logs for the Call-ID
- Look for: SIP error codes, timeouts, Diameter failures, disconnection reasons
- **CRITICAL:** If a downstream node has NO record of the Call-ID, the request never reached it. This means the problem is message delivery, not message processing. Do NOT investigate application logic at intermediate nodes until you have confirmed whether the request reached its destination.

**For registration failures:** Check P-CSCF and S-CSCF logs for the REGISTER transaction.

**Note:** You do NOT have access to RAN (gNB) or UE containers. You can infer RAN/UE state from core-side metrics: `ran_ue` (connected UEs at AMF), `gnb` (connected gNBs), and `amf_session` (active sessions). If `ran_ue = 0` or `gnb = 0`, the RAN or UEs are unreachable — report this as an observed symptom from the core's perspective.
