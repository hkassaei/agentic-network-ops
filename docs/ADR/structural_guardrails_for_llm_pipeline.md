...
**Date:** 2026-04-30
**Last revised:** 2026-05-01 — implementation underway. PRs 1, 2, 4, 5, 5.5a, 5.5b, and 3 shipped (refactor + Decision D + Decision A extended + Decision E + E hardening + Decision F). Build sequence reordered post-PR-2 based on real-run evidence; Decision A expanded to include an IG-statement linter sub-check; **Decision G added** (mechanism-claim grounding) after `run_20260501_022351_data_plane_degradation` exposed an NA fabrication failure mode; **Decision H added** (NA direct-vs-derived ranking) after `run_20260501_032822_call_quality_degradation` exposed an NA-stage ranking failure that none of D / A / E catches. The Decision E hardening PRs (5.5a moving EvidenceValidator after re-investigation; 5.5b structured DiagnosisReport + post-emit pool-membership validation) and Decision F (confidence cap) shipped in the same session as Decision E.
...
The remaining sequence (after PRs 5, 5.5a, 5.5b, and 3 shipped) is:
1. **PR 9 — Decision H.** NA direct-vs-derived ranking guardrail. Closes the wrong-NF-ranking failure mode exposed by `run_20260501_032822_call_quality_degradation` (a `direct` flag on the actual culprit lost to a `derived` flag on a downstream-symptom NF, NA mis-ranked, the wrong NF survived investigation, Synthesis ratified with high confidence). Now the highest-priority remaining residual: PR 3's confidence cap is the *symptom* mitigation (high → medium when evidence is mixed), but H is the *cause* fix (rank the right NF as h1 in the first place).
2. **PR 6 — Decision C.** Multi-shot Investigator consensus.
3. **PR 8 — Decision G.** Mechanism-claim grounding. Cheaper variant of the KB-anchored pattern Decision B uses; sequenced before B because B subsumes G's machinery and G can ship sooner with a simpler regex+KB-lookup implementation.
4. **PR 7 — Decision B.** Typed probe selection from KB. Heaviest, last.
...
| **PR 9** | **H** | medium | NA direct-vs-derived ranking guardrail. Closes the wrong-NF-ranking failure exposed by `run_20260501_032822_call_quality_degradation`. Requires KB metadata (`flag_kind: "direct" \| "derived" \| "cross_layer"` on every metric entry the screener can flag) — a coverage audit is needed before H ships, similar to G's. Now the highest-priority remaining residual: F caps the confidence symptom; H fixes the cause (rank the right NF as h1) |
