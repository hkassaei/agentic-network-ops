...
**Status:** End-to-end branch-first causal reasoning is live. Loader, query, agent tools, and all four v6 prompts know about branches, mechanism, source_steps, observable_metrics, and discriminating_from. A new reverse-lookup tool (`find_chains_by_observable_metric`) and 56 Symptom→FlowStep edges are in Neo4j. Remaining: the structured `failure_modes` migration in `flows.yaml` (design-only, deferred). See [Implementation manifest](#implementation-manifest) for specifics.
...
### Shipped (2026-04-23 — end-to-end branch roundtrip)

- **Loader rewrite for branch-first causal chains** (`network_ontology/loader.py`). `load_causal_chains` now persists `branch`, `mechanism`, `source_steps`, `observable_metrics`, and `discriminating_from` as properties on Symptom nodes for cascading entries. Immediate entries accept the full field vocabulary actually used in YAML (`symptom`/`source`/`at`/`affected`/`from`/`to`/`note`/`description` in addition to `metric`/`log`). `diagnostic_approach` is JSON-serialized (was previously stored as Python repr strings, which was silently corrupting structure). `convergence_point` and `hypothesis_testing` are persisted as chain-level properties.
- **`Symptom -[:DERIVES_FROM_STEP]-> FlowStep`** edges materialized in a new `link_symptoms_to_flow_steps` pass that runs after `load_flows`. 56 edges across 30 distinct flow steps after the first reseed; malformed or unresolvable refs are logged as warnings, not fatal.
- **Query layer reshape** (`network_ontology/query.py`). `get_causal_chain` and `get_causal_chain_for_component` now return an agent-legible branch-first JSON: `observable_symptoms.{immediate, cascading}` with cascading being the list of branches, plus deserialized `diagnostic_approach` as structured dicts. Empty fields are pruned per entry.
- **Reverse-lookup query + agent tool** (`query.py::find_chains_by_observable_metric` and `agentic_ops_common/tools/causal_reasoning.py::find_chains_by_observable_metric`). Given a metric name or substring, returns every causal-chain branch whose `observable_metrics` declares it, with the branch's `mechanism`, `observable_metrics`, and resolved `flow_steps` from `DERIVES_FROM_STEP` edges. Wired into NetworkAnalyst, InstructionGenerator, Investigator, and OntologyConsultationAgent.
- **Prompt updates** for branch-first reasoning:
  - `ontology_consultation.md`: "How to read a causal chain" section explaining the branch structure; explicit instruction that `_unaffected` / `_unchanged` branches are anti-hallucination rule-outs to surface prominently, not footnote.
  - `network_analyst.md`: new mandatory call — `find_chains_by_observable_metric(<metric>)` for strongly-deviated metrics before committing hypotheses. New reasoning principle #9 — "Anchor hypotheses in authored causal-chain branches" — with explicit handling of negative branches, `discriminating_from` hints, and `source_steps` references.
  - `instruction_generator.md`: new rule #10 — "Branch-anchored probes" — turn `observable_metrics`, `discriminating_from`, and negative branches into probe specs. New "Flow and chain tools for plan construction" section documents `find_chains_by_observable_metric`.
  - `investigator.md`: tool list expanded; new "Causal-chain branch grounding" subsection instructing the Investigator to locate its hypothesis's branch, probe against its `observable_metrics`, and use negative branches as falsification anchors for compound-fault detection.
- **Neo4j reseeded**: `docker compose up --build ontology-loader` succeeds; 12 chains, 56 Symptom→FlowStep edges, 0 unresolved refs. Agent-facing `get_causal_chain` returns the full branch-first structure end-to-end (verified against `hss_unreachable`, `subscriber_data_store_unavailable`, and the reverse lookup on `cdp:timeout`, which correctly surfaces all three negative branches — `cx_unaffected`, `hss_cx_unaffected`, `nf_control_plane_unaffected`). All 133 tests pass.

### Not yet started

- **`implementation_ref` fields on flow steps.** No flow step carries one today. Policy (per this ADR): author `{file, symbol}`, omit line numbers. Ordering of rollout: start with the flows we've already rewritten (`vonr_call_setup`, `ims_registration`, `vonr_call_teardown`), then expand. This is its own item — unrelated to the causal-chain branch work that just shipped.
- **Structured `failure_modes.{when, action, observable}` migration in `flows.yaml`.** See the dedicated section below — not started (the earlier claim that N5 steps already used this shape was stale; a grep confirms every `failure_modes` entry in the file today is a plain arrow-separated string). This is a larger piece of work with its own design questions and is scoped separately. The loader is now ready to accept both shapes once flows.yaml migrates — but only because the causal-chain loader work above proved out the "loader understands structured objects in YAML" pattern. Extending that to `failure_modes` is a follow-on, not a new design.

---

## Follow-on work

- **`implementation_ref` rollout on flow steps.** The loader is now structured to accept arbitrary nested objects on YAML entries (proved with the causal-chain branches). Annotate the already-audited IMS flows first (`vonr_call_setup`, `ims_registration`, `vonr_call_teardown`) with `{file, symbol}` refs, extend the loader to persist them on FlowStep nodes, and surface them in `get_flow()` output. This removes the "plausible but unverifiable" quality from flow content the agent currently has to take on faith.
